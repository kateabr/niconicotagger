    ...
    <b-row v-if="tagInfoLoaded() && isActiveMode()">
      <b-table-simple hover class="mt-1 col-lg-12">
        ...
        <b-tbody v-if="!allInvisible()">
          ...
          >
            <b-td>
              <div v-if="value.songEntry != null && !value.songEntry.tagInTags">
                <b-form-checkbox
            ...
            <b-td>
              ...
              <b-link
                target="_blank"
                :to="getNicoVideoUrl(value.video.contentId)"
              ...
              >
                ...
                >
                  <nico-embed-iframe
                    ...
                    :content-id="value.video.contentId"
                  />
                </b-card>
              </b-collapse>
            </b-td>
            <b-td>
              <div v-if="value.songEntry != null">
                <a
                  target="_blank"
                  :href="getVocaDBEntryUrl(value.songEntry.id)"
                  v-html="value.songEntry.name"
                ></a>
                <b-link
                  target="_blank"
                  :to="getVocaDBEntryUrl(value.songEntry.id)"
                >
                  <b-badge
                    class="badge text-center ml-2"
                    :variant="
                      getSongTypeColorForDisplay(value.songEntry.songType)
                    "
                  >
                    {{ getShortenedSongType(value.songEntry.songType) }}
                  </b-badge>
                </b-link>
              ...
              <div v-else>
                <b-button
                  size="sm"
                  :disabled="fetching"
                  :href="getVocaDBAddSongUrl(value.video.contentId)"
                  target="_blank"
                  >Add to the database
                </b-button>
                <div
                ...
                >
                  ...
                  <b-link
                    target="_blank"
                    :to="getVocaDBArtistUrl(value.publisher.id)"
                    >{{ value.publisher.name.displayName }}</b-link
                  >
                </div>
              </div>
            </b-td>
            <b-td>
              <div v-if="value.songEntry != null">
                <b-button-toolbar key-nav>
                  ...
                  <b-button
                    v-else
                    :id="getButtonId(value.songEntry.id)"
                    :disabled="defaultDisableCondition()"
                    class="btn"
...
import NicoEmbedIframe from "@/components/NicoEmbedIframe.vue";

@Component({ components: { NicoEmbedIframe } })
export default class extends Vue {
  ...
  private getButtonId(songId: number): string {
    return getUniqueElementId("assign_", songId.toString());
  }
  ...
  private async assign(id: number): Promise<void> {
    ...
    try {
      ...
      let songEntry = null;
      songEntry = this.videos.filter(video => {
  ...
  private loadPage(page: number): void {
    this.fetch(this.tagNameFrozen, this.scopeTagStringFrozen, (page - 1) * this.maxResults, page);
  }
