    ...
    fun `NND video with associated VocaDB entry mapping test (for tagging)`() {
        ...
        val video =
            Instancio.of(NndVideoData::class.java)
                ...
                .create()
        val songEntry =
            Instancio.of(VocaDbSongEntryWithTags::class.java).withNullable(root()).setBlank(field("tags")).create()
        val videoTags =
            Instancio.ofMap(String::class.java, NndTagType::class.java)
                .generate(root()) { gen ->
        ...
        class TestData : ArgumentsProvider {
            private fun perfect(
                ...
                return if (oneDayEvent)
                    EventDateBounds(publishDate.truncatedTo(DAYS), null, true) to
                        DispositionRelativelyToDate(PERFECT, 0, SUCCESS)
                else
                    EventDateBounds(publishDate.truncatedTo(DAYS), publishDate.truncatedTo(DAYS).plus(3, DAYS), true) to
                        DispositionRelativelyToDate(PERFECT, 0, SUCCESS)
            }
            ...
            override fun provideArguments(context: ExtensionContext?): Stream<out Arguments> {
                ...
                val video =
                    Instancio.of(NndVideoData::class.java)
                        ...
                        .set(field("tags"), nndTagMappings.keys.toList())
                        .create()
                val songEntry = Instancio.of(VocaDbSongWithReleaseEvents::class.java).withNullable(root()).create()
                ...
                val description = Instancio.of(String::class.java).withNullable(root()).create()
                val publisher =
                    if (songEntry == null) Instancio.of(PublisherInfo::class.java).withNullable(root()).create()
                    else null
                            ...
                            NndVideoWithAssociatedVocaDbEntryForEvent(
                                ...
                                songEntry?.let { entry ->
                                    SongEntryWithReleaseEventInfo(
                                        ...
                                        entry.type,
                                        entry.artistString,
                                        entry.events.map { event -> ReleaseEvent(event.id, event.name, event.seriesId) },
                                    )
                                },
                                publisher,
                                publishDate,
