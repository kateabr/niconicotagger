...
class JsonFilterTest {
    ...
    fun `json filter creation test`(request: VideosByNndTagsRequestBase, expectedObject: SearchFilter) {
        assertThat(createNndFilters(request)).usingRecursiveComparison().isEqualTo(expectedObject)
    }

    companion object {
        class TestData : ArgumentsProvider {
            override fun provideArguments(context: ExtensionContext?): Stream<out Arguments> {
                return VideosByNndTagsRequestBase::class
                    .sealedSubclasses
                    .flatMap {
                        when (it) {
                            VideosByNndTagsRequest::class ->
                                listOf(
                                    argumentSet(
                            ...
                            VideosByNndEventTagsRequest::class -> {
                                val dateRestrictedOneDay =
                                    Instancio.of(VideosByNndEventTagsRequest::class.java)
                                        ...
                                        .create()
                                val dateRestrictedSeveralDays =
                                    Instancio.of(VideosByNndEventTagsRequest::class.java)
                                        .set(field(EventDateBounds::class.java, "applyToSearch"), true)
                                        .create()
                                val noDateRestrictions =
                                    Instancio.of(VideosByNndEventTagsRequest::class.java)
                                        .set(field(EventDateBounds::class.java, "applyToSearch"), false)
                                        .create()
                                listOf(
                                    argumentSet(
                                        VideosByNndEventTagsRequest::class.simpleName +
                                            ", date-restricted search, one-day event",
                                        ...
                                        AndFilter(
                                            listOf(
                                                ...
                                                RangeFilter(
                                                    "startTime",
                                                    dateRestrictedOneDay.dates.from.minus(7, DAYS),
                                    ...
                                    argumentSet(
                                        VideosByNndEventTagsRequest::class.simpleName +
                                            ", date-restricted search, several-day event",
                                        ...
                                        AndFilter(
                                            listOf(
                                                ...
                                                RangeFilter(
                                                    "startTime",
                                                    dateRestrictedSeveralDays.dates.from.minus(1, DAYS),
