...
use crate::web::dto::{
    ...
    DisplayableTag, EventAssigningResult, MinimalEvent, NicoResponse, NicoResponseWithScope,
    NicoVideo, NicoVideoWithTidyTags, SongEntriesForTagRemoval, SongEntryForTagRemoval,
...
impl<'a> Client<'a> {
    ...
    pub fn vocadb_beta(cookies: &'a [String]) -> Result<Client<'a>> {
        ...
        R: DeserializeOwned,
    {
        ...
        debug!("{:?}", body_string);
        let json = serde_json::from_slice(&body).context("Unable to deserialize response")?;
    ...
    pub async fn get_videos(
        ...
        while let Some(trimmed) = new_scope.trim_start().strip_prefix("OR") {
            new_scope = String::from(trimmed);
        }
    ...
    pub async fn add_tags(&self, tags: Vec<TagBaseContract>, song_id: i32) -> Result<()> {
        ...
        let _response = self
            .http_put::<Vec<TagBaseContract>, Vec<TagUsageForApiContract>>(
                &format!("{}/api/users/current/songTags/{}", self.base_url, song_id),
                &vec![],
                new_tags,
            )
    ...
    pub async fn fill_in_event(
        &self,
        song_id: i32,
        event: MinimalEvent,
    ) -> Result<EventAssigningResult> {
    ...
    pub async fn add_event_participation_info(
        &self,
        song_id: i32,
        event: MinimalEvent,
    ) -> Result<()> {
            ...
            Ok(
                SongForApiContractSimplifiedWithMultipleEventInfoSearchResult {
                    items: response
                        .items
                        .iter()
                        .map(|item| SongForApiContractSimplifiedWithMultipleEventInfo {
                            id: item.id,
    ...
    pub async fn get_videos_from_db_before_since(
        ...
        let response_entries_small: &[SongForApiContract] =
            response_entries[0..min(max_results as usize, response_entries.len())].as_ref();
        let mut mapped_response = vec![];

        let res_len = response_entries_small.len();

        if res_len > 0 {
            if res_len == max_results as usize {
                timestamp_last = response_entries_small[res_len - 1].create_date.clone();
            }
    ...
    pub async fn process_songs_with_thumbnails(
        ...
        for song in songs.items {
            let tag_ids: Vec<i32> = song.song.tags.iter().map(|t| t.tag.id).collect();
            let th_ok =
                self.map_thumbnail_tags(&song.thumbnails_ok, &mappings, &mapped_tags, &tag_ids);
