...
class AggregatingController(private val service: AggregatingService) {
    ...
    suspend fun getReleaseEventWithLinkedTags(@Valid @RequestBody request: GetReleaseEventRequest): ReleaseEventWithVocaDbTagsResponse {
        return service.getReleaseEventWithLinkedTags(request)
    }

    @PostMapping(value = ["/videos/by_nnd_tags/for_tagging"])
    suspend fun getVideosByNndTagsForTagging(@Valid @RequestBody request: VideosByNndTagsRequest): VideosByNndTagsResponseForTagging {
        return service.getVideosByNndTags(request)
    }

    @PostMapping(value = ["/videos/by_nnd_tags/for_event"])
    suspend fun getVideosByNndTagsForEvent(@Valid @RequestBody request: VideosByNndEventTagsRequest): VideosByNndTagsResponseForEvent {
        return service.getVideosByEventNndTags(request)
    }

    @PostMapping(value = ["/videos/by_vocadb_tag"])
    suspend fun getVideosByVocaDbTags(@Valid @RequestBody request: VideosByVocaDbTagRequest): VideosByVocaDbTagResponse {
